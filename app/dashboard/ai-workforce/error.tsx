...
interface AIWorkforceErrorProps {
  error: Error & { digest?: string };
  reset: () => void;
}

export default function AIWorkforceError({
  error,
  reset,
}: AIWorkforceErrorProps) {
  useEffect(() => {
    console.error("GrowthAI AI workforce error:", error);
  }, [error]);
    ...
    <main className="flex min-h-screen items-center justify-center bg-[#06090F] px-5 text-white">
      <section className="w-full max-w-lg rounded-3xl border border-white/10 bg-white/[0.025] p-8 text-center shadow-2xl backdrop-blur-2xl">
        ...
        <h1 className="mt-3 text-2xl font-semibold tracking-tight sm:text-3xl">
          AI workforce could not load.
        </h1>

        <p className="mx-auto mt-4 max-w-md text-sm leading-6 text-white/40">
          Something interrupted your AI workforce workspace.
          Please try again to reload your agents and workforce data.
        </p>
